...
class GameType extends Stage
{
	...
	public Scene gameDifficulty()
	{
		...
		hardMaze.setOnAction(e->{
			...
			mazemenu.getExit().setOnAction(exit->{mazemenu.close();setScene(gameType());mazemenu.closeThreads();});
			mazemenu.getBack().setOnAction(exit->{mazemenu.close();show();mazemenu.closeThreads();});
